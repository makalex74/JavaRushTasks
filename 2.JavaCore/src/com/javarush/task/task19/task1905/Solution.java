...
public class Solution {
    ...
    public static class DataAdapter implements RowItem{
        ...
        public String getCountryCode() {
            String res = null;
            for (HashMap.Entry item: countries.entrySet()) {
        ...
        public String getDialString() {
            String codNum = contact.getPhoneNumber().substring(0,contact.getPhoneNumber().indexOf('('));
            String phoneNum = contact.getPhoneNumber().substring(contact.getPhoneNumber().indexOf('('));
            phoneNum = phoneNum.replace('(',' ').replace(')',' ').replace('-',' ');
            String[] phoneNumArr = phoneNum.split(" ");
            phoneNum = "";
            for (int i = 0; i < phoneNumArr.length; i++) {
                phoneNum += phoneNumArr[i];
            }
            return "callto://"+codNum+phoneNum;
        }
    }
