...
public class Solution implements Serializable {
    ...
    /*
    Метод readResolve вызывается, когда ObjectInputStream прочитал объект из потока и готовится вернуть его вызывающему объекту.
    ObjectInputStream проверяет, определяет ли класс объекта метод readResolve . Если метод определен, то вызывается метод readResolve ,
    позволяющий объекту в потоке назначить объект, который будет возвращен. Возвращаемый объект должен иметь тип, совместимый со всеми
    видами использования. Если он не совместим, то при обнаружении несоответствия типов будет выдан символ ClassCastException .
     */
